...
// Lucide badge-check — scalloped seal with check inside
...
function AvatarUnknown({ size = 36 }) {
  return (
    <div className="avatar-circle avatar-unknown" style={{ width: size, height: size }}>
      <svg width={Math.round(size * 0.78)} height={Math.round(size * 0.78)} viewBox="0 0 24 24" fill="currentColor" stroke="none">
        <path d="M12 12a4.5 4.5 0 1 0-4.5-4.5A4.5 4.5 0 0 0 12 12Zm0 1.5c-3.6 0-7.5 1.8-7.5 5.4V21h15v-2.1c0-3.6-3.9-5.4-7.5-5.4Z" />
      </svg>
    </div>
  );
}

/* Figma: icon=avatar_filled (3:1330) — light-blue circle + soft head/shoulders silhouette */
function IconAvatarFilled({ size = 32 }) {
  return (
    <span
      className="ies-avatar-filled"
      style={{ width: size, height: size, display: "inline-grid", placeItems: "center", borderRadius: "50%", overflow: "hidden", flex: "0 0 auto", background: "rgb(208,228,250)" }}
    >
      <svg width={size} height={size} viewBox="0 0 32 32">
        <circle cx="16" cy="16" r="16" fill="rgb(208,228,250)" />
        <g fill="rgb(228,241,255)">
          <circle cx="16" cy="13" r="5.5" />
          <path d="M4 32c0-4.6 5.4-7.5 12-7.5s12 2.9 12 7.5v2H4Z" />
        </g>
      </svg>
    </span>
  );
...
/* Figma: icon=avatar_abholer (3:1337) — purple circle + snowflake glyph */
function IconAvatarAbholer({ size = 32 }) {
  return (
    <span style={{ width: size, height: size, borderRadius: "50%", background: "rgb(134,85,246)", display: "inline-grid", placeItems: "center", flex: "0 0 auto" }}>
      <svg width={Math.round(size * 0.62)} height={Math.round(size * 0.62)} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
/* Figma: icon=avatarCaller (3:1333) — green circle + headphones */
function IconAvatarCaller({ size = 32 }) {
  return (
    <span style={{ width: size, height: size, borderRadius: "50%", background: "rgb(20,200,96)", display: "inline-grid", placeItems: "center", flex: "0 0 auto" }}>
      <svg width={Math.round(size * 0.6)} height={Math.round(size * 0.6)} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 14v-2a9 9 0 0 1 18 0v2" />
        <path d="M21 14v4a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2Z" fill="white" />
        <path d="M3 14v4a2 2 0 0 0 2 2h1a2 2 0 0 0 2-2v-3a2 2 0 0 0-2-2Z" fill="white" />
      </svg>
    </span>
  );
}

/* Patient badge — colored circle (green) with text label like "III" or "0" */
function IconAvatarPatient({ size = 32, label = "II", outline = false }) {
  const bg = outline ? "white" : "rgb(16,185,129)";
  const color = outline ? "rgb(40,49,62)" : "white";
  const border = outline ? "2px solid rgb(173,189,204)" : "none";
  return (
    <span style={{
      width: size, height: size, borderRadius: "50%",
      background: bg, border, color,
      display: "inline-grid", placeItems: "center", flex: "0 0 auto",
      fontWeight: 500, fontSize: Math.round(size * 0.44),
...
/* Unknown badge — soft blue-grey circle with circle-help glyph */
function IconAvatarHelp({ size = 32 }) {
  return (
    <span style={{ width: size, height: size, borderRadius: "50%", background: "rgb(230,237,245)", display: "inline-grid", placeItems: "center", flex: "0 0 auto" }}>
      <svg width={Math.round(size * 0.6)} height={Math.round(size * 0.6)} viewBox="0 0 24 24" fill="none" stroke="rgb(173,189,204)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="10" />
...
function AvatarPhoto({ size = 36, src }) {
  if (src) {
    return (
      <span
  ...
  // Fallback: soft portrait-style placeholder
  return (
    <div
      ...
      style={{
        width: size,
        height: size,
        background: "radial-gradient(circle at 50% 40%, rgb(255,222,200) 0%, rgb(255,212,193) 65%, rgb(220,160,140) 100%)",
        position: "relative",
        overflow: "hidden",
      }}
    >
      <svg width={size} height={size} viewBox="0 0 36 36" style={{ position: "absolute", inset: 0 }}>
        <ellipse cx="18" cy="14" rx="8" ry="9" fill="rgba(140,90,70,0.55)" />
        <ellipse cx="18" cy="15" rx="6.5" ry="7.5" fill="rgba(255,228,205,0.95)" />
        <path d="M3 36 Q3 25 18 25 Q33 25 33 36 Z" fill="rgba(180,120,100,0.5)" />
      </svg>
    </div>
  );
}
...
Object.assign(window, {
  ...
  IconList, IconGrid, IconArrowUpDown, IconArrowUp, IconMore, IconEye, IconCircle, IconBadgeCheck,
  AvatarCaller, AvatarAbholer, AvatarUnknown, AvatarVermisst, AvatarPatient, AvatarPhoto,
  IconAvatarFilled, IconAvatarAbholer, IconAvatarCaller, IconAvatarPatient, IconAvatarHelp,
  IESLogo,
});
